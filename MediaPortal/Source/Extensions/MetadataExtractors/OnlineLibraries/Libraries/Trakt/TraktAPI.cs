...
using System.Net;
using System.Web;
using MediaPortal.Extensions.OnlineLibraries.Libraries.Trakt.Data;
...
namespace MediaPortal.Extensions.OnlineLibraries.Libraries.Trakt
{
  ...
  public class TraktAPI
  {
    ...
    public static IEnumerable<TraktLibraryMovies> GetAllMoviesForUser(string user, bool syncDataOnly)
    {
      ...
      //Get the library
      string moviesForUser =
        Transmit(string.Format(TraktURIs.UserMoviesAll, user, syncDataOnly ? @"/min" : string.Empty),
    ...
    public static IEnumerable<TraktLibraryShow> GetLibraryEpisodesForUser(string user, bool syncDataOnly)
    {
      ...
      string showsForUser =
        Transmit(string.Format(TraktURIs.UserEpisodesCollection, user, syncDataOnly ? @"/min" : string.Empty),
                 GetUserAuthentication());
    ...
    public static IEnumerable<TraktLibraryShow> GetWatchedEpisodesForUser(string user, bool syncDataOnly)
    {
      ...
      string showsForUser =
        Transmit(string.Format(TraktURIs.UserWatchedEpisodes, user, syncDataOnly ? @"/min" : string.Empty),
                 GetUserAuthentication());
      TraktLogger.Debug("Response: {0}", showsForUser);
      // if we timeout we will return an error response
      TraktResponse response = showsForUser.FromJSON<TraktResponse>();
    ...
    public static IEnumerable<TraktLibraryShow> GetUnSeenEpisodesForUser(string user, bool syncDataOnly)
    {
      ...
      string showsForUser =
        Transmit(string.Format(TraktURIs.UserEpisodesUnSeen, user, syncDataOnly ? @"/min" : string.Empty),
                 GetUserAuthentication());
      TraktLogger.Debug("Response: {0}", showsForUser);
      // if we timeout we will return an error response
      TraktResponse response = showsForUser.FromJSON<TraktResponse>();
    ...
    {
      string userCalendar = Transmit(string.Format(TraktURIs.UserCalendarShows, user, startDate, days),
                                     GetUserAuthentication());
    ...
    {
      string watchedEpisodes = Transmit(string.Format(TraktURIs.UserEpisodeWatchedHistory, user),
                                        GetUserAuthentication());
    ...
    public static TraktActivity GetFriendActivity(List<ActivityType> types, List<ActivityAction> actions, long start,
                                                  long end, bool includeMe)
    {
      ...
      string activity = Transmit(string.Format(apiUrl, activityTypes, activityActions, startEnd),
                                 GetUserAuthentication());
    ...
    public static TraktActivity GetCommunityActivity(List<ActivityType> types, List<ActivityAction> actions, long start,
                                                     long end)
    {
      // get comma seperated list of types and actions (if more than one)
      string activityTypes = types == null ? "all" : string.Join(",", types.Select(t => t.ToString()).ToArray());
      ...
      string activity = Transmit(string.Format(TraktURIs.ActivityCommunity, activityTypes, activityActions, startEnd),
                                 GetUserAuthentication());
      return activity.FromJSON<TraktActivity>();
    }
    ...
    public static TraktActivity GetUserActivity(string username, List<ActivityType> types, List<ActivityAction> actions)
    {
      ...
      string activity = Transmit(string.Format(TraktURIs.ActivityUser, username, activityTypes, activityActions),
                                 GetUserAuthentication());
      return activity.FromJSON<TraktActivity>();
    }
    ...
    public static IEnumerable<TraktMovie> GetRecommendedMovies(string genre, bool hidecollected, bool hidewatchlisted,
    ...
    public static IEnumerable<TraktShow> GetRecommendedShows(string genre, bool hidecollected, bool hidewatchlisted,
                                                             int startyear, int endyear)
    {
      var traktRecommendationPost = new TraktRecommendations
        {
    ...
    {
      string response = Transmit(string.Format(TraktURIs.SearchUsers, HttpUtility.UrlEncode(searchTerm)),
                                 GetUserAuthentication());
      return response.FromJSONArray<TraktUserProfile>();
    }
    ...
    {
      string response = Transmit(string.Format(TraktURIs.SearchEpisodes, HttpUtility.UrlEncode(searchTerm)),
                                 string.Empty);
    ...
    {
      string response =
        Transmit(string.Format(TraktURIs.RelatedMovies, title, hidewatched ? "/hidewatched" : string.Empty),
                 GetUserAuthentication());
      return response.FromJSONArray<TraktMovie>();
    }
    ...
    {
      string response =
        Transmit(string.Format(TraktURIs.RelatedShows, title, hidewatched ? "/hidewatched" : string.Empty),
                 GetUserAuthentication());
      return response.FromJSONArray<TraktShow>();
    }
    ...
    internal static void ClearLibrary(TraktClearingModes mode, ProgressDialog progressDialog, bool clearSeen)
    {
      ...
      progressDialog.ShowDialog(ProgressDialog.PROGDLG.Modal, ProgressDialog.PROGDLG.NoMinimize,
                                ProgressDialog.PROGDLG.NoTime);
        ...
        if (clearSeen)
        {
          ...
          if (watchedEpisodes != null)
          {
            foreach (var series in watchedEpisodes.ToList())
            {
              ...
              TraktResponse response = SyncEpisodeLibrary(BasicHandler.CreateEpisodeSyncData(series),
                                                          TraktSyncModes.unseen);
              LogTraktResponse(response);
        ...
        else
        {
          ...
          else
          {
            ...
            if ((response is TraktSyncResponse))
            {
              ...
              TraktLogger.Info(message, (response as TraktSyncResponse).Inserted,
                               (response as TraktSyncResponse).AlreadyExist, (response as TraktSyncResponse).Skipped);
            }
          }
          return true;
        }
